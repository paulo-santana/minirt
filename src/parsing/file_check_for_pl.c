...
/*   Updated: 2022/04/17 20:51:05 by fbafica          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	update_parameters(char **tokens, t_parameters *p)
{
	...
	object->color[0] = (double) ft_atof(tokens[11]) / 255.0;
	object->color[1] = (double) ft_atof(tokens[13]) / 255.0;
	object->color[2] = (double) ft_atof(tokens[15]) / 255.0;
	object->diameter = 0;
	object->height = 0;
	object->next = NULL;
...
static int	validate_line(char **tokens)
{
	...
	if (check_normalized(tokens[6], tokens[8], tokens[10]) == -1)
		return (send_error("Error\npl parameter: Bad normalization"));
	return (0);
}
